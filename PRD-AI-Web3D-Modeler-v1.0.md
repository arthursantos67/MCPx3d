...
**Implementation note (`OpenAICompatibleProvider`, 2026-09-22):** Implemented in `packages/agent/src/openai-compatible-provider.ts`. Added after Issues #26-#28's own end-to-end verification surfaced a real gap this section's design already anticipated but left unimplemented: this development machine has no WebGPU-capable GPU, so `WebLLMProvider` can never leave its `unsupported` state here, with no path forward for a user in that situation. `WebLLMProvider` remains the required default (this section's own "required" marking is unchanged) -- `OpenAICompatibleProvider` is a user-selected alternative, chosen explicitly via a settings panel (`apps/web`, tracked separately), never on by default. This entry has no numbered issue in `ISSUES-AI-Web3D-Modeler-v1.0.md` (it wasn't part of the original plan document, and no `FE-17`-linked issue exists there either) -- flagging that choice here rather than inventing a number that might collide with real future numbering. Concrete decisions:

- `POST {baseUrl}/chat/completions` with the user's own key as `Authorization: Bearer {apiKey}` and `response_format: { type: "json_schema", json_schema: { name: "model_plan", schema } }` -- the standard OpenAI "Structured Outputs" shape, best-effort (not `strict: true`, which requires every property listed in `required` with optionality expressed as nullable unions, a stricter shape than `packages/domain`'s schemas use). **Revised after live testing against Groq** (2026-09-22, this project's documented BYOK recommendation): the first pass used `{ type: "json_object" }` with no schema at all, reasoning that not every "OpenAI-compatible" endpoint implements `json_schema` mode. A real user hit this immediately -- with only the natural-language system prompt to go on, the model (`openai/gpt-oss-120b` via Groq) regularly produced JSON missing required fields entirely (no top-level `intent`), which `generateModelPlan`'s ajv validation correctly rejected every time, including after its one-shot repair retry (Issue #19's repair loop retries the *prompt*, not the *request shape* -- it cannot fix a systematically-wrong response format on its own). Passing the real schema doesn't change that validation is the actual enforcement boundary (unchanged) -- it materially improves the odds a model passes it the first time. Groq's own docs confirm `json_schema` support (`console.groq.com/docs/structured-outputs`).
- `fetchImpl` defaults to `fetch.bind(globalThis)`, not bare `fetch` -- also found via the same live testing. Native fetch requires its receiver to be `globalThis`; storing an unbound reference on `this.fetchImpl` and calling it as `this.fetchImpl(...)` (as `generateStructured` does) rebinds `this` to the provider instance instead, throwing "Illegal invocation" (Chrome) / "'fetch' called on an object that does not implement interface Window" (Firefox) on the very first real request. `fetchImpl` is otherwise constructor-injected (mirroring `WebLLMProvider`'s `detectWebGpu`/`createWorker`/`createEngine` DI), so `packages/agent/tests/openai-compatible-provider.test.ts` never makes a real network call -- which is exactly why this bug shipped past the suite: every test passes an explicit fake, never exercising the real default, and Node's own `fetch` (undici) doesn't enforce the receiver check the browser does, so a Node-based regression test for this specific failure mode would not be meaningful. Both issues above were only found by testing against a real browser and a real endpoint, not by this package's unit tests. A non-2xx response's thrown error includes the response status and body text only -- never the request's `Authorization` header -- so the user's key cannot leak into a chat error message or a log line (§11.7).
- Its own state (`getState()`/`onStateChange()`) is a smaller union than `WebLLMProvider`'s -- `idle`/`ready`/`generating`/`error`, no `loading`/`unsupported` -- since there is no model download step and an incomplete config is simply reported as `error` with a human-readable reason. `initialize()` only checks config completeness; it deliberately does not make a test network call (avoids spending the user's own free-tier quota just to "ping" -- a real connectivity/auth failure surfaces naturally on the first `generateStructured` call instead, the same place a WebGPU/model-load failure already surfaces).
- `cancel()` uses a per-request `AbortController`, giving this provider the same FR-33 cancellation behavior `WebLLMProvider.cancel()`/`interruptGenerate` already provides, even though no chat UI wires a cancel button to either provider yet.
- **Revision (2026-09-22):** The request now also sends `max_tokens`/`max_completion_tokens` (both names -- providers vary on which one a given model expects, and an unrecognized field is harmlessly ignored), defaulting to `2048` when the caller doesn't specify one, instead of leaving it unset. Found live: a real request against Groq hit `429 rate_limit_exceeded` ("tokens per minute (TPM): Limit 8000, ... Requested 7098") on a single call -- `openai/gpt-oss-120b`'s implicit default completion budget alone was large enough to nearly exhaust the free tier's entire per-minute allowance, even though an actual ModelPlan response is at most a few hundred tokens. Capping it costs nothing (still generous headroom above any real ModelPlan) and materially reduces how often a free-tier BYOK user hits this.
- **Revision (2026-09-22), still hitting the same limit after the cap above:** A second live request (five objects already in the scene, not the empty-scene case above) still hit `429` with an even larger "Requested" figure. Two further reductions: (1) `reasoning_effort: "low"` is now sent -- `openai/gpt-oss-120b` is a reasoning model that can reserve a large *hidden* chain-of-thought token budget on top of (not capped by) `max_completion_tokens`, a real cost for a task this structurally simple; ignored by models/endpoints that don't recognize the field. (2) The schema passed via `response_format.json_schema.schema` now has every `description` key stripped before being sent (`stripDescriptions`) -- pure human-readable prose, already covered by this package's own natural-language system prompt, but a real ~18% of the schema's serialized size as authored (measured: 4957 -> 4076 chars on `model-plan.v1.schema.json`) that a token-metered model pays for on every single request regardless of conversation state. Local ajv validation (`generate-model-plan.ts`) is unaffected -- it still validates against the original, untouched schema; only this provider's own wire payload is trimmed. Checked and ruled out as *not* a fix: switching to a different Groq model -- every model on Groq's free tier that supports `json_schema` structured outputs (`openai/gpt-oss-120b`, `openai/gpt-oss-20b`, `qwen/qwen3.8-27b`) shares the same 8K-tokens-per-minute cap (`console.groq.com/docs/rate-limits`), so this is a systemic free-tier constraint to reduce request size against, not a model-choice problem.
- **Revision (2026-09-22), dropped `max_tokens` entirely:** With Groq's free tier still too constrained even after the two reductions above, the user asked about a different BYOK provider. Google's Gemini free tier documents a far larger per-minute token budget than Groq's; its OpenAI-compat endpoint (`https://generativelanguage.googleapis.com/v1beta/openai/`) was checked live (a real `curl` request, no valid key needed to find this) and rejects a request that sets **both** `max_tokens` and `max_completion_tokens` with `400 INVALID_ARGUMENT: "max_tokens and max_completion_tokens cannot both be set"` -- Groq silently tolerates both, but not every OpenAI-compatible endpoint does. Sending only `max_completion_tokens` (dropping `max_tokens` entirely) was confirmed live to pass Gemini's request validation (the same `curl` check then failed only on the deliberately-fake API key, i.e. everything else about the request shape is accepted). `max_completion_tokens` is kept as the one field sent since it is what both Groq and current OpenAI expect; older endpoints that only understand the legacy `max_tokens` name remain a known gap, not fixed here.
- **Revision (2026-09-22), trailing-slash base URL:** The Gemini config handed to the user includes the trailing slash Google's own docs show (`.../v1beta/openai/`); `OpenAICompatibleProvider` concatenated `baseUrl + "/chat/completions"` unconditionally, so a base URL already ending in `/` produced a double slash. Confirmed live (`curl`) that Gemini's endpoint 404s a double-slash path *without* CORS headers on the error response -- in a real browser (reported live, Firefox) this surfaces as an opaque `NetworkError when attempting to fetch resource`, not a readable HTTP error, since a browser cannot read a cross-origin response missing `Access-Control-Allow-Origin` regardless of status code. `curl` alone could not have caught this: CORS is a browser-enforced policy, invisible to a plain HTTP client. The constructor now strips any trailing slash(es) from `baseUrl` before storing it, so the endpoint URL is correct regardless of whether the user includes one.
- **Revision (2026-09-22), full endpoint input and transport failures:** Provider settings now accept either a provider base URL or the complete `/chat/completions` endpoint. The provider normalizes both forms to one endpoint instead of appending the path twice; Gemini returns that duplicated path without CORS headers, which Firefox exposes only as `NetworkError`. Network and non-2xx provider failures are now non-repairable request errors: they are shown immediately with an actionable message and are not incorrectly classified as malformed model output or retried with the same unreachable URL.
- **Revision (2026-09-23), transient provider failures:** Direct REST calls now retry only transient HTTP responses (`408`, `429`, `500`, `502`, `503`, and `504`) at most twice after the initial request. Delays use exponential backoff with jitter and honor `Retry-After` when supplied. Permanent client errors are still surfaced immediately. If all attempts receive `503` or `429`, the UI receives a concise actionable message rather than the provider's raw JSON error payload.
- **Revision (2026-09-23), canonical model colors:** Provider output is normalized before schema validation when a `color` is an equivalent hexadecimal representation: uppercase digits are lowercased, an omitted `#` is restored, and 3-digit hex is expanded to 6 digits. This deterministic normalization preserves the requested color, keeps the strict canonical schema/backend contract, and avoids spending the only model repair attempt on formatting alone. The system prompt now states the canonical format explicitly; genuinely invalid colors still fail, but their diagnostic excludes unrelated `oneOf` operation-branch errors.
...
**Implementation note (Issue #64, 2026-09-24):** `x3d_adapter.apply_model_spec` uses the pinned upstream `compose_scene` workflow tool for compatible objects, producing one ordered transport call for the full scene rather than the granular reset-plus-node sequence. Objects requiring transform scale or material transparency use the established granular path because that upstream workflow cannot represent those fields. Both paths retain the same final schema/semantic validation and never commit a partial candidate. `X3DMcpClient.transport_call_count` and `tests/test_mcp_integration.py` record the composed fixture's build calls and elapsed duration against the granular baseline.
...
**Implementation note (Issue #63, 2026-09-24):** Artifact routes capture a `RevisionSnapshot` before their first asynchronous MCP operation, so content, filename and requested revision always describe the same immutable revision even if a newer commit completes concurrently. The in-memory cache is bounded by `artifact_cache_max_entries` (default 200) and `artifact_cache_ttl_seconds` (default 3600); its `(project, revision, format)` key coalesces concurrent HTML/VRML generation while already-validated X3D is served directly. Expired or deleted projects remove their cache entries.
...
**Implementation note (Issue #37, 2026-09-24):** The chat exposes a Cancel generation action only while a request is active. `ChatController` uses one request-scoped `AbortController` to interrupt the selected provider and the plan-application fetch; completion after an abort is ignored, so no partial plan can reach the API and the last valid preview/revision remains active. Backend MCP connection and call cancellation propagates as cancellation rather than being converted to `MCP_UNAVAILABLE`.
...
- Responsive/narrow-screen behavior was added later by Issue #33, preserving this Issue #25 desktop shell while switching to tabs at narrow widths.
...
**Implementation note (Issue #33, 2026-09-23):** `WorkspaceShell` now keeps the desktop layout side by side and switches at `820px` to accessible Chat/3D Preview tabs. The active panel fills the remaining fixed-viewport workspace, so the chat composer remains available in the Chat tab and the viewer keeps its `240px` minimum height in the Preview tab. The narrow top bar wraps its controls, workspace children can shrink (`min-width: 0`), and the document has no horizontal overflow; only the status bar may scroll its own compact status content horizontally.
...
**Implementation note (Issue #42, 2026-09-24):** `apps/api/tests/test_mcp_integration.py` runs against the pinned MCP service started by the session fixture and covers all MVP primitives, schema/semantic validation, standalone X3DOM generation, unavailable transport and session isolation. Focused regressions enforce fail-closed semantic-report parsing, the pinned `.x3dj` limitation and cancellation propagation.
...
**Implementation note (Issue #28, 2026-09-22):** Wires `GET /api/projects/{id}/artifacts/html` (this table's row above) to HTTP as `apps/api/src/api/routes/artifacts.py`, needed for the preview iframe to have anything real to fetch (`preview.url` from `POST /plans`, Issue #22, already pointed here). The other four artifact/manifest rows in this table remain unwired -- Issue #30's scope. Full detail (why this route rebuilds X3D from `ModelSpec` on every request rather than reading cached content, and what it deliberately does not cover) is in §10.3's Issue #28 implementation note, to avoid the same explanation in two places.
...
**Implementation note (Issue #28, 2026-09-22):** Implemented as `apps/web/src/viewer/X3DPreviewFrame.tsx` (plus `X3DPreviewFrame.css` and `objectUrl.ts`), following this section's preferred MVP flow exactly: `fetch`es the current revision's standalone HTML from `previewUrl` (the `preview.url` `POST /plans` returns, Issue #22, resolved to an absolute URL), embeds it via a Blob URL (never `srcDoc`/`dangerouslySetInnerHTML` with the raw HTML string -- keeping generated content out of the parent DOM per §11.4 by construction, not by convention), and revokes the previous Blob URL only after the new one replaces it. `packages/viewer` (this section's originally suggested package location) remains an unimplemented placeholder for now -- see its README's note on why the component lives in `apps/web/src/viewer/` instead. Concrete decisions:

- **This issue's own acceptance criteria ("current validated revision renders") had a real backend gap**: `preview.url` already pointed at `GET /api/projects/{id}/artifacts/html`, and `apps/api/src/api/artifacts.py`'s `build_html_artifact` (Issue #11) already built the content, but no route mounted it -- `apps/api/src/api/main.py` only registered `health`/`projects`/`plans` routers. Fixed by adding `apps/api/src/api/routes/artifacts.py` (one route, `GET /{project_id}/artifacts/html?revision=N`), pure HTTP wiring over already-existing module logic, the same shape Issues #21/#22 already used to wire #7/#8/#10 to HTTP. It does not read cached X3D content (none is stored -- `ProjectSessionService` only holds `ModelSpec`, PRD §3.12): it rebuilds X3D from the project's current, already-committed-therefore-already-valid `ModelSpec` via `build_and_validate_candidate` (Issue #10) on each request. `/artifacts/x3d`, `/x3dj`, `/x3dv`, and `/manifest` remain unwired -- that is Issue #30 (download menu)'s scope, not #28's.
- `BlobUrlTracker` (`objectUrl.ts`) is a small DI'd class (an injectable `ObjectUrlFactory` in place of `URL.createObjectURL`/`revokeObjectURL`) so the create-then-revoke-previous sequencing is unit-testable with `node --test` without a real browser Blob registry (`apps/web/tests/viewer/objectUrl.test.ts`).
- The iframe uses `sandbox="allow-scripts"` only -- no `allow-same-origin` -- the minimum X3DOM needs to execute at all, kept as tight as the rest of §11.4's spirit calls for (NFR-08).
- A fetch failure shows an error overlay without clearing whatever was already rendered, so a bad request doesn't blank the last valid scene -- not itself Issue #29's revision-badge/status polish, but built so #29 does not have to fight this component's default behavior.
- Verified against a real running `apps/api` + `x3d_mcp` stack via `curl` (create project -> apply a `create_object` plan -> `GET /artifacts/html?revision=1` returned real X3DOM HTML, `200`, `text/html`) and, separately, via the `objectUrl.test.ts` unit tests. Rendering the fetched HTML inside a real browser iframe end-to-end (i.e. actually seeing the cube) was not exercised in this pass -- doing so needs a completed chat turn, which needs a working local model, unavailable on this development machine (see the Issue #27 note above).
...
- The original minimal `summarizeModelSpec` context is now replaced by Issue #34's bounded, more complete summarizer; nothing here depends on its exact text format.
...
**Revision (2026-09-22):** Once a real model was actually tested against this prompt (the BYOK provider, live against Groq), it over-applied the ambiguity rule above: asked a `clarify` question for a plain "create a table" with no dimensions given, rather than just picking reasonable default dimensions and proceeding. The ambiguity rule was written for *targeting* an existing ambiguous/duplicate-named part (FR-08/UC-05) and was never meant to cover *missing measurements* on a brand-new object, but the prompt didn't say so explicitly. `RULES` in `system-prompt.ts` now says so: exact dimensions if given, infer from a stated goal/purpose if one is given (e.g. "big enough for six people"), otherwise use ordinary real-world defaults and proceed -- `clarify` stays reserved for when the model genuinely cannot proceed at all (ambiguous target, or a request too vague to decompose into primitives), never for missing exact measurements alone.

**Revision (2026-09-22), language matching:** The user asked that the agent reply in whichever language they write to it in. `RULES` now includes "Write any free text you produce (a clarify question, or a no_change reason) in the same language the user's most recent message is written in." This covers the only model-generated free text a user actually sees in the chat UI today -- `Clarify.question` renders directly as an assistant message; `NoChange.reason` is generated but not currently surfaced anywhere in `apps/web`'s UI, included here only for when it eventually is. This does **not** localize `apps/web`'s own static strings (`ChatController.ts`'s "Updated the model to revision N...", `canSend()`'s gate reasons, `ProviderSettings.tsx`'s copy, etc.) -- those are ordinary app UI text, not LLM output, and localizing them would need real i18n infrastructure (language detection or a settings toggle, translation strings) that wasn't asked for and isn't part of this change.
...
**Implementation note (Issue #34, 2026-09-23):** `summarizeModelSpec` now bounds prompt context to configurable character and object limits (defaults: 4,000 characters and 32 objects), while retaining each included object's id, name, kind, dimensions, position, rotation, scale, and color. It deliberately omits project/revision/session and renderer state. `assembleModelPlanMessages` is the shared composition path for every `generateModelPlan` request: it uses a configurable, user/assistant-only recent-turn window (default 8), removes a trailing duplicate of the current user request, and then inserts the current request exactly once. Unit tests cover the bounded summary and a clarification-style history.

**Implementation note (Issue #35, 2026-09-23):** The system prompt now directs chair/table/shelf requests to stable, named primitive parts, limits common furniture plans to 12 operations and all plans to 100, and defines positive axes, center-based positions, and floor-standing placement. `tests/golden/multipart-furniture.json` provides table and chair structural fixtures; the agent test validates named IDs/parts and operation counts through the normal ModelPlan validation path.
