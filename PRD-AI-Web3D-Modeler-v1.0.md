...
- `POST {baseUrl}/chat/completions` with the user's own key as `Authorization: Bearer {apiKey}` and `response_format: { type: "json_object" }` -- not the newer schema-aware `json_schema` mode, since not every "OpenAI-compatible" endpoint implements it; `generateModelPlan`'s existing ajv validation against the real ModelPlan schema (Issue #19), plus its one-shot repair retry, is what actually enforces shape regardless of which JSON mode the remote endpoint honors.
- `fetchImpl` is constructor-injected (mirroring `WebLLMProvider`'s `detectWebGpu`/`createWorker`/`createEngine` DI), so `packages/agent/tests/openai-compatible-provider.test.ts` never makes a real network call. A non-2xx response's thrown error includes the response status and body text only -- never the request's `Authorization` header -- so the user's key cannot leak into a chat error message or a log line (§11.7).
